...
fn cef_sandbox_library_available() -> bool {
  ...
  parent.join(relative_path).is_file()
}
...
impl<T: UserEvent> CefRuntime<T> {
  fn init(
    ...
    // CEF's ICU initialization consults NSBundle before settings are applied.
    // In development, make a minimal app bundle and re-exec from it so the
    // framework resources are discoverable in the same layout as production.
    #[cfg(target_os = "macos")]
    {
      ...
      if !already_bundled {
        ...
        let _ = std::fs::remove_file(&cef_link);
        ...
        let _ = std::fs::remove_file(&bundle_resources);
        std::os::unix::fs::symlink(real_framework.join("Resources"), &bundle_resources)
...
<key>CFBundleIdentifier</key><string>com.muse.desktop</string>
...
<key>CFBundleVersion</key><string>1.0.0</string>
    ...
    #[cfg(target_os = "macos")]
    {
      ...
      settings.resources_dir_path = real_framework
        .join("Resources")
        .to_string_lossy()
        .as_ref()
        .into();
